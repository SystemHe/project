...
namespace PWMS.PerForm
{
    public partial class F_ManFile : Form
    {
        ...
        private void Sut_Save_Click(object sender, EventArgs e)
        {
            ...
            {
                //通过DataClass公共类中的getcom方法查询当前职工是否添加了个人简历
            ...
            else    //如果当前是职工基本信息选项卡
            {
                ...
                if (hold_n == 1 || hold_n == 2)//判断当前是添加还是修改操作
                {
                    ...
                    //用MyModule公共类中的Part_SaveClass方法组合添加或修改的SQL语句
                    ...
                    {
                        //如果ADDS不为空，则通过MyDataClass公共类中的getsqlcom方法进行添加、修改操作
                        MyDataClass.getsqlcom(ModuleClass.MyModule.ADDs);
                    }
                }
                if (Ima_n > 0)//如果图片标识大于0
                {
                    //通过MyModule公共类中的SaveImage方法将图片保存到数据库
        ...
        {
            if (dataGridView1.RowCount < 2)//判断dataGridView1中是否有数据
            {
                MessageBox.Show("数据表为空不能删除！");
                return;
            }
        ...
        private void N_First_Click(object sender, EventArgs e)  //第一条
        {
            ...
            {
                dataGridView1.CurrentCell = this.dataGridView1[ColInd, 0];//将当前控件的索引设置为0
                MyMC.Ena_Button(N_First, N_Previous, N_Next, N_Cauda, 0, 0, 1, 1);
            }
            if ((((Button)sender).Name) == "N_Previous")    //判断当前单击的是不是“前一条”
            {
                if (dataGridView1.CurrentCell.RowIndex == 0)//判断当前索引行是否为0
                {
                    //调用公共类中的方法设置四个按钮的状态
                    MyMC.Ena_Button(N_First, N_Previous, N_Next, N_Cauda, 0, 0, 1, 1);
                }
            ...
            if ((((Button)sender).Name) == "N_Next")//判断当前单击的是否是“下一条”
            {
                //判断当前索引行是否是最后一行
            ...
            if ((((Button)sender).Name) == "N_Cauda")//判断当前单击的是否是“最后一行”
            {
        ...
        private void Sub_Table_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < MyDS_Grid.Tables[0].Rows.Count; i++)
            {
                ...
                wordDoc.Tables[1].Rows.HeightRule = Microsoft.Office.Interop.Word.WdRowHeightRule.wdRowHeightAtLeast;
                ...
                //设置表格样式
                wordDoc.Tables[1].Borders[Microsoft.Office.Interop.Word.WdBorderType.wdBorderLeft].LineStyle = Microsoft.Office.Interop.Word.WdLineStyle.wdLineStyleSingle;
                wordDoc.Tables[1].Borders[Microsoft.Office.Interop.Word.WdBorderType.wdBorderLeft].LineWidth = Microsoft.Office.Interop.Word.WdLineWidth.wdLineWidth050pt;
                wordDoc.Tables[1].Borders[Microsoft.Office.Interop.Word.WdBorderType.wdBorderLeft].Color = Microsoft.Office.Interop.Word.WdColor.wdColorAutomatic;
